...
import { CreateMaterialDocument, CreateSkuDocument, GetAllMaterialsQuery, GetAllMaterialsQueryVariables, GetAllMerksDocument, GetAllUnitMeasuresDocument, UpdateMaterialDocument } from "../../graphql/inventory.generated";
...
const EditMaterialModal: React.FC<EditMaterialModalProps> = ({
  ...
  const [updateMaterial] = useMutation(UpdateMaterialDocument);
  ...
  const handleEditMaterial: SubmitHandler<CreateMaterialValues> = async (data) => {
    if (!row) return;
  ...
  useEffect(() => {
    if (row) {
      reset({
        ...
        merk: row.merk._id,
        unit_measure: row.unit_measure._id,
        status: row.status,
        minimum_unit_measure: row.minimum_unit_measure._id,
        conversion: row.conversion,
        item_category: row.item_category._id
      })
    }
  }, [row])
    ...
    >
      <Box sx={modalStyle}>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Button
            onClick={handleSubmit(handleEditMaterial)}
            variant="contained"
            color="primary"
            disabled={isSubmitting}
          >
