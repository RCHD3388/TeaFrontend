...
import {
  EmployeeRoleType,
  RequestItem_ItemType,
  RequestItemType,
  RequestStatusType,
} from "../../../types/staticData.types";
import {
  AddNewDetailPtDocument,
  GetAllPurchaseOrdersDocument,
  GetPurchaseTransactionByIdDocument,
  HandleWaitingPoDocument,
  RemovePurchaseTransactionDetailDocument,
  UpdatePurchaseTransactionDocument,
} from "../../../graphql/purchasing.generated";
...
import PurchasingToolInput, {
  ToolDetailPurchasingInput,
} from "./PurchaseToolInput";
import {
  GetAllMaterialsDocument,
  GetAllSkusDocument,
} from "../../../graphql/inventory.generated";
import { GenerateReportPurchasingDocument } from "../../../graphql/purchasing.generated";
...
const DetailPurchasing: React.FC<DetailPurchasingProps> = ({}) => {
  ...
  const { data, loading, error, refetch } = useQuery(
    GetPurchaseTransactionByIdDocument,
    { variables: { id: purchasingId, requiresAuth: true } }
  );
  let {
    data: supplierData,
  ...
  } = useQuery(GetAllSuppliersDocument, {
    variables: {
      filter: { status: true },
      requiresAuth: true,
    },
  ...
  let {
    ...
    error: poError,
    refetch: poRefetch,
  } = useQuery(GetAllPurchaseOrdersDocument, {
    variables: {
      filter: { status: true },
      requiresAuth: true,
    },
  ...
  } = useQuery(GetAllMaterialsDocument, {
    variables: {
      filter: { status: true },
      requiresAuth: true,
    },
  ...
  // Lazy query for generating the report
  const [generateReport, { loading: reportLoading }] = useLazyQuery(
    GenerateReportPurchasingDocument,
    {
      variables: { transactionId: purchasingId },
      onCompleted: (data) => {
        if (data?.generateReportPurchasing) {
          const pdfData = `data:application/pdf;base64,${data.generateReportPurchasing}`;
          setPdfURL(pdfData);
        }
      },
      onError: (error) => {
  ...
  const [updatePurchaseTransaction] = useMutation(
    UpdatePurchaseTransactionDocument
  );
  const [removePurchaseTransactionDetail] = useMutation(
    RemovePurchaseTransactionDetailDocument
  );
  ...
  const getItemDetail = (id: String, type: String) => {
    if (type == RequestItem_ItemType.MATERIAL) {
      let material = getRawData().materials.find((material: any) => {
      ...
      let tool = getRawData().tools.find((tool: any) => {
        return tool._id == id;
      });
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<UpdatePurchasingTransactionValues>({
    defaultValues: {
      supplier: "",
  ...
  const handleUpdateData = async (data: UpdatePurchasingTransactionValues) => {
    ...
    try {
      ...
      await updatePurchaseTransaction({
        variables: {
          id: purchasingId,
          updatePurchaseTransactionInput: input,
          requiresAuth: true,
        },
      });
      refetch();
      dispatch(
        openSnackbar({
          severity: "success",
          message: "Berhasil memperbarui data transaksi pembelian",
        })
      );
    } catch (error: any) {
      let msg = GetBadReqMsg(
        "Gagal memperbarui transaksi pembelian, silakan coba lagi nanti",
        error
      );
  ...
  const handleRemoveButton = async (id: String) => {
    ...
    try {
      ...
      let msg = GetBadReqMsg(
        "Gagal menghapus data detail transaksi, silakan coba lagi nanti",
        error
      );
      dispatch(openSnackbar({ severity: "error", message: String(msg) }));
    } finally {
  ...
  const handleAddPurchasingMaterial = async (
    material: any,
    quantity: number,
    price: number
  ) => {
  ...
  const handleAddPurchasingTool = async (data: ToolDetailPurchasingInput) => {
    ...
    let input = {
      ...
      item_type: RequestItem_ItemType.TOOL,
      item: data.sku,
      quantity: 1,
      price: Number(data.price),
      purchase_order: poInput.value,
  ...
  const handleAddNewPTDetail = async (input: any) => {
    ...
    try {
      ...
      dispatch(
        openSnackbar({
          severity: "success",
          message: "Berhasil manambahkan data detail transaksi",
        })
      );
    } catch (error: any) {
      let msg = GetBadReqMsg(
        "Gagal manambahkan data detail transaksi, silakan coba lagi nanti",
        error
      );
      dispatch(openSnackbar({ severity: "error", message: String(msg) }));
    } finally {
  ...
  useEffect(() => {
    if (data) {
      reset({
        supplier: {
          value: getData().supplier._id,
          label: `${getData().supplier.name} - ${
            getData().supplier.person.name
          }`,
        },
        ...
        {!loading && !error && (
          <Box maxHeight={800} overflow={"auto"}>
            <Container sx={{ mt: 3 }}>
              <Card sx={{ my: 3 }}>
                <CardContent>
                  ...
                  <Controller
                    ...
                    control={control}
                    rules={{ required: "Kode Transaksi tidak boleh kosong" }}
                    render={({ field }) => (
                      <TextField
                        {...field}
                        color="secondary"
                        ...
                        helperText={
                          errors.transaction_number
                            ? errors.transaction_number.message
                            : ""
                        }
                      />
                    )}
                  ...
                  <Controller
                    name="supplier"
                    control={control}
                    rules={{ required: "Supplier tidak boleh kosong" }}
                    render={({
                      ...
                      <Autocomplete
                        ...
                        options={
                          supplierLoading || !supplierData
                            ? []
                            : supplierData.getAllSuppliers.map((sup: any) => {
                                return {
              ...
              <Modal
                open={openModal}
                onClose={handleCloseModal} // Closes the modal
              ...
              >
                ...
                >
                  ...
                  {pdfURL ? (
                    ...
                    >
                      <p>
                        PDF tidak dapat ditampilkan. Unduh dengan klik tombol di
                        bawah ini.
                      </p>
                    </object>
                  ) : (
                    <Typography>Memuat laporan...</Typography>
                  )}
                </Box>
              </Modal>
              ...
              <Box pt={2}>
                <Typography variant="h6" component="h2">
                  <b>Daftar Pembelian Barang</b>
                </Typography>
                <Box overflow={"auto"} maxHeight={300} sx={{ mb: 1 }}>
                  <table className="table table-xs border-2">
                    <thead
                      className="bg-secondary text-white font-normal text-base"
                    ...
                    <tbody>
                      {data &&
                      getData().purchase_transaction_detail.length > 0 ? (
                        getData().purchase_transaction_detail.map(
                        ...
                        <tr className="p-4">
                          ...
                          >
                            Tidak ada data dalam tabel. Silakan tambahkan
                            material sebelum submit.
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                <Box py={2} px={3}>
                  <Autocomplete
                    ...
                    options={
                      poLoading || !poData
                        ? []
                        : poData.getAllPurchaseOrders.map((po: any) => {
                            return {
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        sx={{ width: "100%", mb: 1 }}
                        error={poInputError.length != 0}
                        helperText={poInputError}
                        color="secondary"
                        size="small"
                      />
                    )}
                  ...
                  <TextField
                    ...
                    variant="outlined"
                    onChange={(e) =>
                      setItemTypeInput(e.target.value as RequestItem_ItemType)
                    }
                    value={itemtypeInput}
                    error={itemtypeInputError.length != 0}
                    helperText={itemtypeInputError}
                  >
                    <MenuItem value={RequestItem_ItemType.MATERIAL}>
                      <div className="badge whitespace-nowrap p-3 gap-2">
                        {RequestItem_ItemType.MATERIAL}
                      </div>
                    </MenuItem>
                    <MenuItem value={RequestItem_ItemType.TOOL}>
                      <div className="badge whitespace-nowrap p-3 gap-2">
                        {RequestItem_ItemType.TOOL}
                      </div>
                    </MenuItem>
                  </TextField>
                  ...
                  {itemtypeInput == RequestItem_ItemType.MATERIAL && (
                    <>
                      <PurchasingMaterialInput
                        materialData={materialData}
                        materialLoading={materialLoading}
                        handleAddPurchasingMaterial={
                          handleAddPurchasingMaterial
                        }
                      />
                    </>
                  ...
                  {itemtypeInput == RequestItem_ItemType.TOOL && (
                    <>
                      <PurchasingToolInput
                        ...
                        skuError={skuError}
                        handleAddPurchasingTool={handleAddPurchasingTool}
                      />
                    </>
                  )}
                </Box>
