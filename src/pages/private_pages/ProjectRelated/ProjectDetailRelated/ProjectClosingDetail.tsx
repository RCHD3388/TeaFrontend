...
interface ProjectClosingDetailProps {
  ...
  ) => Promise<ApolloQueryResult<FindAllProjectsQuery>>;
}

const ProjectClosingDetail: React.FC<ProjectClosingDetailProps> = ({
  dataProject,
  refetchProject,
}) => {
  const getData = () => {
  ...
  const handleUpdateNote = async () => {
    try {
      ...
      let msg = GetBadReqMsg(
        "Gagal memperbarui data, silakan coba lagi nanti",
        err
      );
  ...
  useEffect(() => {
    if (dataProject) setCurrentNote(getData()?.note || "");
  }, [dataProject]);
    ...
    <Container>
      <div className="flex justify-end">
        <Button
          ...
          onClick={() => {
            dispatch(
              setPageValueByName({ name: "project_main_page", value: 0 })
            );
          }}
        >
          Kembali
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: "bold" }}>
            Data Penyelesaian Proyek
          </Typography>
          <Box sx={{ p: 2 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              <Typography variant="body1" sx={{ mr: 2 }}>
                <b>Disetujui oleh:</b> {getData()?.closed_by?.person.name} ({" "}
                {getData()?.closed_by?.person.email} )
              </Typography>
            </Stack>
            <Stack direction="row" spacing={1} alignItems="center">
              <Typography variant="body1" sx={{ mr: 2 }}>
                ...
                {getData()
                  ? formatDateToLong(
                      getData()?.request_project_closing.handled_date
                    )
                  : ""}
              </Typography>
            </Stack>
            ...
            <Typography variant="h6" sx={{ my: 2 }}>
              <b>Dokumen Penyelesaian Proyek</b>
            </Typography>
            <Box sx={{ pl: 2 }} display={"flex"} flexDirection={"column"}>
              <Stack direction="row" spacing={1} alignItems="center">
                {getData()?.document ? (
                  <Typography variant="body1" sx={{ mr: 2 }} color="success">
                    <b>
                      Dokumen sudah di upload. Anda dapat upload ulang dokumen
                      jika dibutuhkan
                    </b>
                  </Typography>
                ) : (
              ...
              <Box sx={{ my: 2 }}>
                <ButtonUpload projectID={dataProject?.findProjectById._id} />
              </Box>
              ...
              <div className="flex">
                <Button
                  ...
                  onClick={() => {
                    navigate(
                      `/appuser/request/closing/${
                        getData()?.requested_from._id
                      }`
                    );
                  }}
                >
                ...
                <Container sx={{ p: 2 }}>
                          ...
                          (item: any, index: number) => (
                            <tr key={index}>
                              ...
                              <td className="text-sm" align="left">
                                {item.material.merk.name}
                              </td>
                              <td className="text-sm" align="left">
                                {item.material.unit_measure.name} ({" "}
                                {item.material.conversion} x{" "}
                                {item.material.minimum_unit_measure.name} )
                              </td>
                              ...
                              <td className="text-sm" align="right">
                                {formatCurrency(item.price * item.remain)}
                              </td>
                            </tr>
                          )
                        )
                  ...
                  <div className="flex justify-end">
                    <Typography variant="body1" sx={{ mr: 2 }}>
                      <b>Total:</b>{" "}
                      {formatCurrency(
                        getData().material_used.reduce(
                          (acc: number, item: any) =>
                            acc + item.price * item.remain,
                          0
                        )
                      )}
                    </Typography>
